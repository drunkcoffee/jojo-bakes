...
export default function App() {
  ...
  function addMochiItem() {
    if ((mochiTier === "classic" || mochiTier === "special") && !ensureDifferentFlavours(selectedMochiOption1, selectedMochiOption2)) {
              ...
              {mochiTier === "premium" ? (
                <div className="choice-grid three-cols">
                  {currentMochiTier.options.map((item) => (
                    <button key={item.id} className={`choice-button ${selectedMochiOption1 === item.id ? "active" : ""}`} onClick={() => setSelectedMochiOption1(item.id)}>
                      {getText(lang, item)}
                    </button>
                  ))}
                </div>
